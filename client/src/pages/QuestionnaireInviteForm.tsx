...
export default function QuestionnaireInviteForm() {
  const [, params] = useRoute("/questionnaire/i/:token");
  const token = params?.token ?? "";

  const [answers, setAnswers] = useState<Record<string, any>>({});
  const [submitted, setSubmitted] = useState(false);

  const query = trpc.questionnaires.getByInvitationToken.useQuery({ token }, { enabled: !!token });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitMutation.mutateAsync({
        token,
        answers,
        respondentName: expert ? `${expert.firstName ?? ""} ${expert.lastName ?? ""}`.trim() : undefined,
      });
      setSubmitted(true);
    } catch (err: any) {
  ...
  if (data.invitation?.status === "completed" && !submitted) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: "#F8FAFC" }}>
        <div className="w-full max-w-md text-center">
          ...
          <p className="text-muted-foreground text-sm">You have already submitted your response for this questionnaire.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen" style={{ background: "#F8FAFC" }}>
      ...
      <main className="max-w-2xl mx-auto px-4 py-10">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-bold text-foreground mt-1 mb-1">{q!.title}</h1>
          {expert && (
            <p className="text-sm text-muted-foreground">
              For: <span className="font-medium text-foreground">{expert.firstName} {expert.lastName}</span>
            </p>
          )}
          {q!.description && <p className="text-muted-foreground text-sm mt-1">{q!.description}</p>}
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            return (
              <div key={question.id} className="bg-white rounded-xl border border-border p-6">
                ...
                {question.questionType === "long_text" && (
                  <Textarea rows={4} placeholder="Your answer…" value={ans ?? ""} onChange={e => setAnswer(question.id, e.target.value)} className="rounded-lg resize-none" />
                )}

                {question.questionType === "yes_no" && (
                  <div className="flex gap-3">
                    {["Yes", "No"].map(opt => (
                      <button key={opt} type="button" onClick={() => setAnswer(question.id, opt)}
                        className={`flex-1 py-2.5 rounded-lg border text-sm font-semibold transition-colors ${ans === opt ? "bg-primary text-white border-primary" : "bg-white text-foreground border-border hover:border-primary/50"}`}>
                        {opt}
                      </button>
                    ))}
                  </div>
                ...
                {question.questionType === "dropdown" && (
                  <div className="space-y-2">
                    {opts.map(opt => (
                      <label key={opt} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${ans === opt ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"}`}>
                        <input type="radio" name={`q-${question.id}`} value={opt} checked={ans === opt} onChange={() => setAnswer(question.id, opt)} className="accent-primary" />
                ...
                {question.questionType === "multi_select" && (
                  <div className="space-y-2">
                    {opts.map(opt => {
                      ...
                      return (
                        <label key={opt} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${checked ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"}`}>
                          <input type="checkbox" checked={checked} onChange={() => toggleMulti(question.id, opt)} className="accent-primary" />
                          <span className="text-sm">{opt}</span>
                        </label>
                      );
                    })}
          ...
          <Button type="submit" disabled={submitMutation.isPending} className="w-full h-11 rounded-xl font-semibold gap-2" style={{ background: "#2563EB" }}>
